...
class YoloTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        # Check if the local path has waypoints
        if len(local_path_msg.waypoints) > 0:

            # Create a LineString from the local path
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if stop_line_ids_on_path:
            ...
            if map_rois:
                ...
                # Keep only the main traffic light classes (0-3)
                mask = classes < 4
                yolo_rois = yolo_rois[mask]
                classes = classes[mask]
                scores = scores[mask]

                # Match YOLO detections with map ROIs
        ...
        for stop_line_id in stop_line_ids_on_path:
            for traffic_light_id, traffic_light_coords in self.traffic_lights[stop_line_id].items():
                ...
                for x, y, z in traffic_light_coords:
                    ...
                    point_camera = do_transform_point(
                        PointStamped(point=point_map),
                        transform
                    ).point
                    ...
                    if (
                            u < 0 or u >= self.camera_model.width or
                            v < 0 or v >= self.camera_model.height
                    ):
                        break
    ...
    def match_map_and_yolo_rois(self, map_rois, yolo_rois, yolo_classes, yolo_scores):
        ...
        for stop_line_id, traffic_light_id, x1_map, x2_map, y1_map, y2_map in map_rois:
            ...
            for idx, (cls, score, yolo_roi) in enumerate(
                    zip(yolo_classes, yolo_scores, yolo_rois)):
            ...
            if matched_roi is None:
                ...
                cl, score, yolo_roi, yolo_idx = matched_roi

                tfl_result.status = CLASS_TO_TLRESULT[cl]
                tfl_result.status_text = CLASS_TO_STRING[cl]
