...
class SimpleCollisionChecker:
    ...
    def __init__(self):
        ...
        if coordinate_transformer == "utm":
            projector = UtmProjector(
                Origin(utm_origin_lat, utm_origin_lon),
                use_custom_origin,
                False
            )
        else:
        ...
        rospy.Subscriber(
            '/detection/traffic_light_status',
            StopLineStatusArray,
            self.traffic_light_status_callback,
            queue_size=1,
            tcp_nodelay=True
        )
    ...
    def traffic_light_status_callback(self, msg):
        with self.lock:
            self.stopline_statuses = {
                status.stop_line_id: status.status
    ...
    def path_callback(self, msg):
        ...
        # Create Shapely LineString from local path waypoints
        ...
        if goal_point is not None:
            ...
            goal_point_shapely = shapely.Point(
                goal_point.x,
                goal_point.y
            )

            if local_path_buffer.intersects(
                    goal_point_shapely.buffer(0.1)
            ):
                collision_points = np.append(
                    collision_points,
                    np.array([(
        ...
        for stop_line_id, stop_line in self.stop_lines.items():
            ...
            if not local_path_linestring.intersects(stop_line):
                continue
            ...
            for x, y in intersection_points:
                collision_points = np.append(
                    ...
                    np.array([(
                        x,
                        y,
                        0.0,  # stop lines are on the ground
                        0.0,
                        0.0,
                        0.0,
